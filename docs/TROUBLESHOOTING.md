...
### `WSDL local no encontrado: app/sunat/wsdl/{mode}/billService.wsdl`

Causa: el `MODE` está mal o no clonaste el repo completo (los WSDL están
bundleados en `app/sunat/wsdl/{beta,prod}/`).

Fix:

- Verifica `MODE=beta` o `MODE=prod` en `.env`.
- Verifica que `ls app/sunat/wsdl/beta/` muestra
...
modificaste `app/ubl/templates/invoice_01.xml.j2` y removiste el bloque:
...
- Si quieres más margen, sube los timeouts en `app/sunat/client.py`. En

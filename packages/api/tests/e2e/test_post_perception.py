...
def test_post_perception_end_to_end():
    # Caveat MODDATOS: RUC 20000000001 no esta en padron de agentes de
    # percepcion, asi que SUNAT respondera fault 1071 ("emisor no autorizado").
    ...
    response = client.post("/v1/perceptions", json=payload)
    ...
    if response.status_code == 200:
        ...
        assert data["serie"] == "P999"
        assert data["numero"] == 9999
        assert data["regimen"] == "02"
        assert data["total_percibido"] is not None
